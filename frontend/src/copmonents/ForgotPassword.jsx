...
import { Email, AlertCircle, ToastSuccess, ToastError, CloseIcon, LoadingSpinner, ArrowLeft } from "./Box";
...
export default function ForgotPassword() {
  ...
  const [errorId] = useState(() => `email-error-${Math.random().toString(36).slice(2)}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data?.message || data?.error || "هذا البريد غير مسجل لدينا");
      }

      showToast(data?.message || "تم إرسال رمز التحقق إلى بريدك الإلكتروني", "success");
      setEmail("");
      setError("");
      setTouched(false);

      setTimeout(() => {
        navigate("/verify-email?type=reset", { state: { email: trimmedEmail } });
      }, 800);
    } catch (err) {
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-slate-50 via-white to-violet-50/30" dir="rtl">
      ...
      <div className="flex-1 flex flex-col px-5 py-8 max-w-lg mx-auto w-full">
        ...
        <div className="bg-white rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-100/80 p-6">
          <form className="space-y-5" onSubmit={handleSubmit} aria-label="استعادة كلمة المرور">
            {/* Email */}
            <div>
              <label className="block text-[16px] font-semibold text-slate-700 mb-2" htmlFor="email">
                البريد الإلكتروني
              </label>
              <div className="relative">
                <span className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" aria-hidden="true">
              ...
              {touched && error && (
                <div id={errorId} role="alert" className="flex items-center gap-1.5 mt-1.5 h-4">
